...
impl Formatter {
    ...
    fn print_column_list_close_paren_symbol(&mut self, symbol: &parser::ast::Symbol) {
        if self.settings.indent_comma_lists.is_none() {
            self.print_new_line();
        }
        self.visit_symbol(symbol);
...
impl Visitor for Formatter {
    ...
    fn visit_symbol_kind(&mut self, kind: parser::ast::SymbolKind) -> Self::Result {
        self.formatted_query += kind.to_string().as_str();
    }

    fn visit_symbol(&mut self, symbol: &parser::ast::Symbol) -> Self::Result {
    ...
    fn visit_keyword_kind(&mut self, kind: parser::ast::KeywordKind) -> Self::Result {
    ...
    fn visit_comparison_operator_kind(
        &mut self,
        kind: parser::ast::ComparisonOperatorKind,
    ) -> Self::Result {
        self.formatted_query += kind.to_string().as_str();
    }
    ...
    fn visit_arithmetic_operator_kind(
        &mut self,
        kind: parser::ast::ArithmeticOperatorKind,
    ) -> Self::Result {
        self.formatted_query += kind.to_string().as_str();
    }
    ...
    fn visit_data_type_numeric_size(&mut self, ns: &parser::ast::NumericSize) -> Self::Result {
    ...
    fn visit_data_type(&mut self, data_type: &parser::ast::DataType) -> Self::Result {
        match data_type {
            DataType::Int(k)
            | DataType::BigInt(k)
            | DataType::TinyInt(k)
    ...
    fn visit_statement(&mut self, stmt: &parser::ast::Statement) -> Self::Result {
        match stmt {
    ...
    fn visit_common_table_expression(
        &mut self,
        cte: &parser::ast::CommonTableExpression,
    ) -> Self::Result {
    ...
    fn visit_local_variable(
        &mut self,
        local_variable: &parser::ast::LocalVariable,
    ) -> Self::Result {
    ...
    fn visit_execute_statement_procedure_parameter_name(
        &mut self,
        name: &parser::ast::ProcedureParameterName,
    ) -> Self::Result {
        self.formatted_query += "@";
    ...
    fn visit_execute_statement_procedure_parameter(
        &mut self,
        param: &parser::ast::ProcedureParameter,
    ) -> Self::Result {
    ...
    fn visit_common_table_expression_statement(
        &mut self,
        stmt: &parser::ast::CommonTableExpressionStatement,
    ) -> Self::Result {
        match stmt {
    ...
    fn visit_expression(&mut self, expr: &parser::ast::Expression) -> Self::Result {
        match expr {
    ...
    fn visit_case_condition(
        &mut self,
        case_condition: &parser::ast::CaseCondition,
    ) -> Self::Result {
        match case_condition {
            parser::ast::CaseCondition::WhenCondition {
                when_kw,
                when_expression,
                then_kw,
                result_expression,
    ...
    fn visit_where_clause(&mut self, where_clause: &parser::ast::WhereClause) -> Self::Result {
        self.increase_indent();
    ...
    fn visit_group_by_clause(
        &mut self,
        group_by_clause: &parser::ast::GroupByClause,
    ) -> Self::Result {
        self.increase_indent();
    ...
    fn visit_order_by_clause(
        &mut self,
        order_by_clause: &parser::ast::OrderByClause,
    ) -> Self::Result {
        self.increase_indent();
    ...
    fn visit_order_by_arg(&mut self, order_by_arg: &parser::ast::OrderByArg) -> Self::Result {
    ...
    fn visit_table_join(&mut self, table_join: &parser::ast::Join) -> Self::Result {
    ...
    fn visit_table_join_condition(
        &mut self,
        table_join_condition: &parser::ast::JoinCondition,
    ) -> Self::Result {
    ...
    fn visit_function_over_clause_window_frame(
        &mut self,
        window_frame: &parser::ast::WindowFrame,
    ) -> Self::Result {
        ...
        match &window_frame.start {
            parser::ast::WindowFrameBound::Preceding(e)
            | parser::ast::WindowFrameBound::Following(e) => {
                self.visit_expression(e);
                self.print_space();
            }
            _ => {}
